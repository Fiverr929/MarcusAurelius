...
window.VisionScan = (function () {
  ...
  function modeSuffix(outputType) {
    if (outputType === 'REMIX') return ' Lean into mood, feeling, and artistic impression over exact technical detail.';
    return ' Be precise and specific. Prioritise measurable, reproducible details.';
  }
  ...
  function buildPrompt(layerName, section, outputType) {
    ...
    return base + PROSE + modeSuffix(outputType);
  }
  ...
  function describe(base64DataUrl, layerName, section, outputType) {
    ...
    var key = 'describe:' + parsed.base64.slice(0, 32) + ':' + section + ':' + layerName;
  ...
  function describeStyle(base64DataUrl, outputType) {
    ...
    var parsed = parseDataUrl(base64DataUrl);
    if (!parsed.base64) return Promise.reject(new Error('[VisionScan] Invalid image data'));
    var key = 'style:' + parsed.base64.slice(0, 32);
    console.log('[VisionScan] describeStyle →');
    return deduped(key, function () {
      return callGemini(apiKey, STYLE_PROMPT + PROSE + modeSuffix(outputType), parsed.base64, parsed.mimeType);
    })
  ...
  function describeRef(base64DataUrl, outputType) {
    ...
    var parsed = parseDataUrl(base64DataUrl);
    if (!parsed.base64) return Promise.reject(new Error('[VisionScan] Invalid image data'));
    var key = 'ref:' + parsed.base64.slice(0, 32);
    console.log('[VisionScan] describeRef →');
    return deduped(key, function () {
      return callGemini(apiKey, REF_PROMPT + PROSE + modeSuffix(outputType), parsed.base64, parsed.mimeType);
    })
